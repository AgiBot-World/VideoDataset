...
FROM registry.agibot.com/ai-platform/lib/python:${PYTHON_VERSION} AS dev
...
# Use mirror
ARG UBUNTU_MIRROR=deb.debian.org
RUN sed -i "s/deb.debian.org/${UBUNTU_MIRROR}/g"  /etc/apt/sources.list.d/debian.sources
